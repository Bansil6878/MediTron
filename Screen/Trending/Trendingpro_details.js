...
export function Trendingpro_details({route}) {

const navigation = useNavigation();
  const { productId } = route.params;
  const [product, setProduct] = useState({});

  const {addItemToCart4} = useContext(CartContext);

  useEffect(() => {
    setProduct(getTrending_data(productId));
  });

  function onAddToCart() {
    addItemToCart4(product.id);
    ...
    <SafeAreaView>
      ...
      <View style={{justifyContent: 'center', marginLeft: 6}}>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <Text style={{marginTop:30,marginLeft:100}}>  ₹: {product.rupees}</Text>
