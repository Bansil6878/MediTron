...
export function Labpro_details({route}) {
  const navigation = useNavigation();
  const {productId} = route.params;
  const [product, setProduct] = useState({});

  const {addItemToCart2} = useContext(CartContext);

  useEffect(() => {
    setProduct(getLab_data(productId));
  });

  function onAddToCart() {
    addItemToCart2(product.id);
    navigation.navigate('Labtest_book');
  }
  {
    console.log(product);
  }
      ...
      <ScrollView>
        ...
        <View style={{justifyContent: 'center', marginLeft: 6}}>
          ...
          <Text style={{fontWeight: 'bold', marginTop: 20}}>
            <Octicons name="graph" size={24} color="blue" /> 20+ people bought
            this recently
          </Text>
